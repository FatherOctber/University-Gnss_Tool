...
#define ID_ISMREDTEC 1394       /* message id: oem6 reduced tec data */
#define ID_ISMRAWTEC 1390       /* message id: oem6 raw tec data */
#define ID_ISMRAWOBS 1389       /* message id: oem6 raw obs data */
#define ID_ISMREDOBS 1393       /* message id: oem6 reduced obs data */
#define ID_BESTPOST 42          /* message id: oem6 BESTPOSITION data */
#define ID_ISMTECCAL 1406          /* message id: oem6 teccalibrate data */
#define ID_SATVIS2 1043          /* message id: oem6 SATVIS2 data */
...
static int obsindex(obs_t *obs, gtime_t time, int sat, int sys)
{
    int i,j;
    
    if (obs->n>=64) return -1;
    for (i=0;i<obs->n;i++) {
...
{
    int nex=84; /* number of extended obs data */
  //  trace(3,"number of extended obs data=%d freq=%d NFREQ=%d code=%d\n",nex,freq,NFREQ,code);
    if (sys==SYS_GPS) {
       ...
       // if (code==CODE_L2X) return nex<2?-1:NFREQ+1;
         if (code==CODE_L1P) return code==CODE_L1P?0:-1;
        if (code==CODE_L2X) return code==CODE_L2X?1:-1;
    }
    else if (sys==SYS_GLO) {
       // if (strstr(opt,"-RL2C")) return code==CODE_L2C?1:-1;
        //if (code==CODE_L2C) return nex<1?-1:NFREQ;
        if (code==CODE_L2C) return code ==CODE_L2C?1:-1;
    }
    else if (sys==SYS_GAL) {
...
static int decode_rangecmpb(raw_t *raw)
{
    ...
    for (i=0,p+=4;i<nobs;i++,p+=24) {
        ...
        sat=satno(sys,prn);
         trace(3,"oem4 rangecmpb satellite number : sys=%d,prn=%d\n",sys,prn,sat);
        ...
        if ((index=obsindex(&raw->obs,raw->time,sat,sys))>=0) {
...
static int decode_rangeb(raw_t *raw)
{
  ...
  //  trace(3,"decode_rangeb: len=%d\n",raw->len);
    ...
    nobs=U4(p);
   //  //trace(3,"oem4 rangeb nobs=%2d",nobs);
    if (raw->outtype)
    {
        msg=raw->msgtype+strlen(raw->msgtype);
        sprintf(msg," nobs=%2d",nobs);
    ...
    for (i=0,p+=4;i<nobs;i++,p+=44) {
        ...
        prn=U2(p);
 //   //trace(3,"oem4 rangeb satellite number : sys=%d,prn=%d pos=%d\n",sys,prn,pos);
        if (sys==SYS_GLO) prn-=37;
       ...
       if (sys==SYS_GLO&&!parity){
              //trace(3,"oem4 rangeb invalid if GLO  error: sys=%d,prn=%d\n",sys,prn);
           continue; /* invalid if GLO parity unknown */
       }
//trace(3,"oem4 rangeb satellite number : sys=%d,prn=%d sat=%d\n",sys,prn,sat);
        psr  =R8(p+ 4);
        adr  =R8(p+16);
        dop  =R4(p+28);
        snr  =R4(p+32);
        ...
        tt=timediff(raw->time,raw->tobs);
         //  //trace(3,"oem4 rangeb tt=%f\n",tt);
         //  //trace(3,"oem4 rangeb index=%d\n",obsindex(&raw->obs,raw->time,sat));
        if (raw->tobs.time!=0) {
        ...
        // //trace(3,"oem4 rangeb index=%d\n",obsindex(&raw->obs,raw->time,sat,sys));
        if ((index=obsindex(&raw->obs,raw->time,sat,sys))>=0) {
            raw->obs.data[index].L  [pos]=-adr;
            raw->obs.data[index].P  [pos]=psr;
            raw->obs.data[index].D  [pos]=(float)dop;
...
static int decode_gloephemerisb(raw_t *raw)
{
    ...
    prn        =U2(p)-37;
    //trace(2,"oem4 gloephemerisb  prn:=%d\n",prn);
    if (raw->outtype)
    {
        msg=raw->msgtype+strlen(raw->msgtype);
    ...
    //if (!strstr(raw->opt,"-EPHALL"))
    //trace(2,"oem4 gloephemerisb prn sav: prn=%d\n",prn);
    {
       if (fabs(timediff(geph.toe,raw->nav.geph[prn-1].toe))<1.0&&
            geph.svh==raw->nav.geph[prn-1].svh) return 0; /* unchanged */
    }
     //trace(2,"oem4 gloephemerisb prn : sat=%d\n",sat);
    if(sat>24) return -1;
    geph.sat=sat;
    if(prn>24) return -1;
    raw->nav.geph[prn-1]=geph;
   raw->ephsat=sat;
    //trace(2,"oem4 gloephemerisb prn : prn=%d\n",sat);
    return 2;
}
...
static int decode_rawobs(raw_t *raw)
{
    unsigned char *p=raw->buff+OEM4HLEN;
    unsigned char buff[27];
    int i,sigch,satid,page;

    //trace(3,"decode_rawobs: len=%d\n",raw->len);
/*printf("decode_RAWWW_tec: len=%d\n",raw->len);*/
page=raw->len-OEM4HLEN-4;
page/=16;
i=0;//
...
static int decode_rawtec(raw_t *raw)
{
    unsigned char *p=raw->buff+OEM4HLEN;
    unsigned char buff[27];
    int i,sigch,satid,page;

    //trace(3,"decode_rawtec: len=%d\n",raw->len);
/*printf("decode_RAWWW_tec: len=%d\n",raw->len);*/
page=raw->len-OEM4HLEN-4;
page/=16;
i=0;
...
p+=1;
 ...
 int satsys=U1(p);p+=1;
 int typSig=U1(p);p+=1;
 int typSig1=U1(p);p+=1;
p+=3;
float tec=R4(p); p+=4;
raw->rawtec[satid][satsys]=tec;
float dtec=R4(p);p+=4;
...
static int decode_redobs(raw_t *raw)
{
    unsigned char *p=raw->buff+OEM4HLEN;
    unsigned char buff[27];
    int i,sigch,satid,page;

    //trace(3,"decode_reduced_obs: len=%d\n",raw->len);
...
while(i<page)
    ...
    int prn1=0;
    prn1=U1(p);
        raw->redobs[i].ucPrn=prn1;
    p+=1;
    //sys
    int sys1=0;
    sys1=I1(p);
        raw->redobs[i].cFreq=sys1;
//trace(3,"decode_reduced_obs: prn=%d ,=%d\n",raw->redobs[i].ucPrn,prn1);
//trace(3,"decode_reduced_obs: sys=%d,=%d\n",raw->redobs[i].ucSvSys,sys1);
    p+=1;
    //freq
        raw->redobs[i].ucSvSys=U1(p);
        //trace(3,"decode_reduced_obs: freq=%d\n",raw->redobs[i].cFreq);
    p+=1;
    //type
    raw->redobs[i].ucSigType=U1(p);
     //trace(3,"decode_reduced_obs: type=%d\n",raw->redobs[i].ucSigType);
    p+=1;
    //az
                      raw->redobs[i].fAzimuth=R4(p);
    p+=4;
    //trace(3,"decode_reduced_obs: fAzimuth=%f\n",raw->redobs[i].fAzimuth);
    //el
                     raw->redobs[i].fElevation=R4(p);
    p+=4;
    //trace(3,"decode_reduced_obs: el=%f\n",raw->redobs[i].fElevation);
    //cn
                     raw->redobs[i].fCNo=R4(p);
    p+=4;
    //loctime
        raw->redobs[i].fLockTime=R4(p);
    p+=4;
     //averstd
       raw->redobs[i].fCMCAvrg=R4(p);
    p+=4;
                     raw->redobs[i].fCMCStd=R4(p);
    p+=4;
                     raw->redobs[i].fS4=R4(p);
    p+=4;
    //trace(3,"decode_reduced_obs: S4=%f\n",raw->redobs[i].fS4);
                     raw->redobs[i].fS4Correction=R4(p);
    p+=4;
    //trace(3,"decode_reduced_obs: fS4Correction=%f\n",raw->redobs[i].fS4Correction);
                     raw->redobs[i].f1SecSigma=R4(p);
    p+=4;
                     raw->redobs[i].f3SecSigma=R4(p);
    p+=4;
                     raw->redobs[i].f10SecSigma=R4(p);
    p+=4;
                     raw->redobs[i].f30SecSigma=R4(p);
    p+=4;
                     raw->redobs[i].f60SecSigma=R4(p);
    p+=4;
i+=1;
    }
    return 10;
...
static int decode_redtec(raw_t *raw)
{
    unsigned char *p=raw->buff+OEM4HLEN;
    unsigned char buff[27];
    int i,sigch,satid,page;

    //trace(3,"decode_reduced_tec: len=%d\n",raw->len);
    ...
    raw->tec_rec.time=raw->time;
    raw->tec_rec.size=page;
    while(i<page)
    {
    satid=U1(p);
    p+=1;
    ...
    float az=R4(p); p+=4;
    float elev=R4(p);p+=4;
    float loctime=R4(p); p+=4;
    float sn0=R4(p); p+=4;
    float tec15=R4(p); p+=4;
    float dtec15=R4(p);p+=4;
    float tec30=R4(p); p+=4;
    float dtec30=R4(p);p+=4;
    float tec45=R4(p); p+=4;
    float dtec45=R4(p);p+=4;
    float tec=R4(p); p+=4;
    float dtec=R4(p);p+=4;
 //trace(3,"decode_reduced_tec:=%f sat=%d sys=%d el=%f az=%f\n",tec,satid,satsys,elev,az);
raw->tec_rec.el[i]=elev;
raw->tec_rec.azim[i]=az;
raw->tec_rec.dtec[i]=dtec;
raw->tec_rec.tec[i]=tec;
raw->tec_rec.sat[i]=satid;
raw->tec_rec.freq[i]=freq;
raw->tec_rec.sys[i]=satsys;
raw->tec_rec.sn[i]=sn0;
    i+=1;
    }
...
/* decode satvis2 ----------------------------------------------------*/
static int decode_satvis2(raw_t *raw)
{
    unsigned char *p=raw->buff+OEM4HLEN;
    unsigned char buff[27];
    int i=0,sigch,satid,page;

    //trace(3,"decode_satvs2: len=%d\n",raw->len);
    // satsystem
sigch  =U4(p); p+=4;
//trace(3,"decode_satvs2: 1 =%d\n",sigch);
// satvis
page  =U4(p); p+=4;
//trace(3,"decode_satvs2: 2 e=%d\n",page);
//almax
sigch  =U4(p); p+=4;
//sat
//trace(3,"decode_satvs2: 3=%d\n",sigch);
page  =U4(p); p+=4;
 //trace(3,"decode_satvs2: 44e=%d\n",page);
if(i<page)
{
//satid
raw->satvis[i].satid =U4(p); p+=4;
//helli
sigch  =U4(p); p+=4;
//elev
      raw->satvis[i].elev=R8(p);p+=8;
      //az
      raw->satvis[i].azi=R8(p);p+=8;
//true dop
      raw->satvis[i].dop1=R8(p);p+=8;
//app dop
      raw->satvis[i].dop2=R8(p);p+=8;
      i+=1;
  }
...
/* decode bestpost ----------------------------------------------------*/
static int decode_teccalibr(raw_t *raw)
{
    unsigned char *p=raw->buff+OEM4HLEN;
    int i;

    trace(3,"decode_callibrate tec: len=%d\n",raw->len);


    int time=U4(p);//time
    p+=4;
    int records=U4(p);//record
    p+=4;
    raw->teccallibrrec=records;
    for(i=0;i<records;++i)
    {
    int signalTipe=U4(p);//tipe signal
    p+=4;
    int coldan=U4(p);//col dan;
    p+=4;
    float tec=R4(p);//tec;
    p+=4;
    float tecstd=R4(p);//tec;
    p+=4;
    raw->teccallibr[i].coldan=coldan;
    raw->teccallibr[i].timecallib=time;
    raw->teccallibr[i].tec=tec;
    raw->teccallibr[i].tecstd=tecstd;
    raw->teccallibr[i].tipe=signalTipe;
    }
    return 21;

}
/* decode bestpost ----------------------------------------------------*/
static int decode_bestpost(raw_t *raw)
{
    unsigned char *p=raw->buff+OEM4HLEN;
    unsigned char buff[27];
    int i,sigch,satid,page;

    //trace(3,"decode_bestpost: len=%d\n",raw->len);
...
*/
  /* get the latitude*/
      ...
      raw->bestpost[0]=R8(p);p+=8;

  /* get the longitude*/
      raw->bestpost[1]=R8(p);p+=8;

  /* get the height (msl)*/
          raw->bestpost[2]=R8(p);p+=8;

  /* get the undulation*/
          float undulat=R4(p);p+=4;

  /* get the datum id.*/
          int datum=U4(p); p+=4;

  /* get the latitude standard deviation*/
          float stdlat=R4(p);p+=4;

  /* get the longitude standard deviation*/
          float stdlon=R4(p);p+=4;

  /* get the height standard deviation*/
  float stdheight=R4(p);p+=4;
  /* get the base station id */
  int stan_id=U4(p); p+=4;
  /* get the diff age*/
         float agen=R4(p);p+=4;

  /* get the solution age*/
float sol_agen=R4(p);p+=4;
         p+=8;
         return 7;
/*
...
static int decode_oem4(raw_t *raw)
{
    ...
    switch (type) {
        ...
        case ID_ISMRAWTEC     : return decode_rawtec(raw);
        case ID_ISMRAWOBS     : return decode_rawobs(raw);
        case ID_ISMREDTEC     : return decode_redtec(raw);
        case ID_ISMREDOBS     : return decode_redobs(raw);
        case ID_BESTPOST      : return decode_bestpost(raw);
        case ID_SATVIS2       : return decode_satvis2(raw);
        case ID_ISMTECCAL       : return decode_teccalibr(raw);
...
extern int input_oem4(raw_t *raw, unsigned char data)
{
    ...
    if (raw->nbyte==0) {
        ...
        /*
        if (sync_nmeagl(raw->buff,data))
        { raw->nbyte=6;
        return 0;
    }
        if (sync_nmeagp(raw->buff,data))
        { raw->nbyte=6;
        return 0;
    }*/
    }
    ...
    if (raw->nbyte==10&&(raw->len=U2(raw->buff+8)+OEM4HLEN)>MAXRAWLEN-4) {
        ...
        //printf("oem4 length error: len=%d\n",raw->len);
        raw->nbyte=0;
        return -1;
    }
