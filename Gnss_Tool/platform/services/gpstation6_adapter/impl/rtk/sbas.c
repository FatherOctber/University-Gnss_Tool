...
static int decode_sbstype26(const sbsmsg_t *msg, sbsion_t *sbsion)
{
    ...
    for (i=0;i<15;i++) {
        ...
        {//trace(4,"decode_sbstype26:j=%d, block=%d,band=%d,max=%d\n",
               //j,block,band,MAXBAND);
            continue;
        }
        ...
        //trace(4,"decode_sbstype26: band=%d block=%d pos=%d lat=%d lon=%d dion=%f ngp=%d\n",band,
        //      block,j,sbsion[band].igp[j].lat,sbsion[band].igp[j].lon,
        //      delay*0.125,sbsion[band].nigp);
                      ...
                      const double *azel, double *delay, double *var)
{
    ...
    *delay*=fp; *var*=fp*fp;
    ...
    //trace(5,"osbsioncinorr: d=%7.2f sig=%7.2f\n",*delay,sqrt(*var));
    return 1;
}
