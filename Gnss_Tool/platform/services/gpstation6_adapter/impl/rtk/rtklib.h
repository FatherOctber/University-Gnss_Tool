...
#define VER_RTKLIB  "2.4.2 b8"          /* library version */
...
typedef struct {        /* observation data record */
    ...
    unsigned char sat,rcv,sys; /* satellite/receiver number */
...
typedef struct {        /* observation data record */
    gtime_t time;       /* receiver sampling time (GPST) */
    int size;
    float time_lock[70];       /* locktime (second) */
    int sat[70],sys[70]; /* satellite/sys number */
    int freq[70]; /* freq  */
    float el[70]; /* observation data elevition (grad) */
    float azim[70]; /* observation data azimut (grad) */
    float sn[70]; /* observation data S/N (grad) */
    float tec[70]; /* observation data tec (tecu) */
    float dtec[70]; /* observation data deltatec (tecu) */
} obsd_tec_r;


typedef struct
{
  unsigned char  solution_status;
  unsigned char  solution_type;
...
typedef struct {        /* observation data tec*/
    int n,nmax;         /* number of obervation data/allocated */
    obsd_tec_r *data;       /* observation data records */
} obs_tec_r;
...
typedef struct {        /* receiver raw data control type */
 ...
 /*   obs_t_r obs_raw; */ /* observation raw_obs (oem6) */
    obsd_tec_r tec_rec;  /* observation tec_gpsSt_reduc (oem6) */
    double rawtec[MAXSAT][2]; /*observation tec_gpsSt_raw (oem6)*/
    double bestpost[3]; /*observation BESTPOST (oem6)*/
    nav_t nav;          /* satellite ephemerides */
    ...
    satvis2 satvis[84];
    ISMREDOBS_CHANNEL_DATA redobs[84];
    ISMTECCALLIBRATE_TYPE_DATA teccallibr[10];
    int teccallibrrec;
} raw_t;
...
extern int  testsnr(int freq, double el, double snr, const snrmask_t *mask);
...
extern void dl_test(gtime_t ts, gtime_t te, double ti, const url_t *urls,
                    int nurl, char **stas, int nsta, const char *dir, FILE *fp);
